import Foundation
        ...
        final class SuplementsDocService {
            static let shared = SuplementsDocService()
            private init() {}

            func obtenerDocSuplementos(completion: @escaping (Result<[SuplementoPayload], Error>) -> Void) {
                guard let url = URL(string: "\(API.baseURL)suplementos/doctor/1") else {
                    print("❌ URL inválida")
